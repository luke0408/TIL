...
ES6 이전에는 JavaScript에서 Block Scope 개념이 없었다. ES6 이후 `let`과 `const`의 도입으로 블록 스코프가 지원되기 시작했다. 
...
**Function Scope는 독립적이다.** 같은 변수 이름을 여러 함수에서 사용할 수 있으며, 서로 간섭하지 않는다. 
...
**전역 변수 사용은 신중해야 한다.** 전역 변수는 어디에서든 접근 가능하기 때문에 **다른 코드에 의해 값이 변경될 위험이 있다.** 가능하면 **함수 내부에서 변수 선언을 제한하여 사용 범위를 좁히는 것이 좋다.** 
...
**자동 전역 변수는 Strict Mode에서 방지된다.** `"use strict";`를 사용하면 선언되지 않은 변수에 값을 할당할 경우 오류가 발생한다. 
...
`let`과 `const`를 사용하면 `window` 오염을 방지할 수 있다. **전역 변수를 최소화하는 것이 좋은 코드 작성 습관이다.** 
